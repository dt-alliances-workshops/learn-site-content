...
Before we get to the problems, let's review [ Dynatrace Information Events](https://docs.dynatrace.com/docs/dynatrace-api/environment-api/events-v2/post-event){target="_blank"}.
...
![lab3 pipeline](img/azure-lab3/lab3-pipeline.png)
...
![lab3 tags](img/azure-lab3/lab3-tags.png)
...
2. Validate Sample App in Browser
    ...
    ![lab3 app backend version 2](img/azure-lab3/lab3-app-backend-version-2.png)
...
3. Review the Problem card in Dynatrace
    * Go to Dynatrace menu -> Problems, and you will probably see two problems that eventually get merged…
        ...
        ![lab3 backend problem](img/azure-lab3/lab3-backend-problem.png)
...
4. Analyze problem - top findings        
    ...
    ![lab3 backend analysis upd](img/azure-lab3/lab3-backend-analysis-upd.png)
...
5. Analyze problem - execution time breakdown
    ...
    ![lab3 backend hotspots upd](img/azure-lab3/lab3-backend-hotspots-upd.png)
...
6. Analyze problem - hot spots
    ...
    ![lab3 backend analysis trace](img/azure-lab3/lab3-backend-analysis-trace.png)

7. Analyze problem impact
    - From the breadcrumb menu, click on the `backend` to open the service page.
    ![lab3 backend breadcrumb](img/azure-lab3/lab3-backend-breadcrumb.png)

    - You can see exactly when the problem started.
    ![lab3 backend problem details upd](img/azure-lab3/lab3-backend-problem-details-upd.png)
...
!!! tip
    ...
    * To review this setting, on the left side menu, click `settings`, click `Anomaly Detection` and the the `Services` setting.
        ...
        ![lab3 global anomoly rules](img/azure-lab3/lab3-global-anomoly-rules.png)

    * To learn more about how Davis AI problems are detected and analyze, click the [ Dynatrace docs ](https://www.dynatrace.com/support/help/how-to-use-dynatrace/problem-detection-and-analysis/problem-detection/automated-multi-dimensional-baselining/){target="_blank"} to learn more
...
2. Validate the Change in Dynatrace
    * From the left side menu, navigate to the `Releases` page.  Here you will see how Dynatrace detected the versions of monitored processes.  It may take a minute, but you will see the version change in the `Real-time inventory` section and a new event in the `Release events` section.
    !!! tip
    📓 Be sure to adjust the management zone to `dt-orders-k8` 

    ![lab3 release order](img/azure-lab3/lab3-release-order.png)

    * If you expand the event, you should see something like this

    ![lab3 release order event](img/azure-lab3/lab3-release-order-event.png)
...
3. Validate Sample App in Browser
    * The event above has the URL back to the sample application, so just click that if you don't have the sample app up already.  You should see `version 3` for the order app now too.
    ...
    ![lab3 app ui order version 3](img/azure-lab3/lab3-app-ui-order-version-3.png)
...
4. Review the Problem in Dynatrace  
    * The problem may take a minute to show up, but this is what the problem will look like once it does. Also, you may see two problems that eventually get merged into one as Dynatrace is performing the problem analysis.
    ...
    1. Deployment event as additional context
        ...
        ![lab3 order problem](img/azure-lab3/lab3-order-problem.png)
