...
![lab4 app architecture](img/azure-grail-lab2/lab4-app-architecture.png)

The picture below shows how the components of the sample application interact with Dynatrace. 

![lab2 setup](img/azure-grail-lab2/lab2-setup.png)
...
!!! tip
    ...
    ![lab4 app architecture future](img/azure-grail-lab2/lab4-app-architecture-future.png)
 
## Deploy the Dynatrace Kubernetes Operator via Azure Portal
<!-- Duration: 7 min -->
!!! tip
    Before starting this step, please ensure you completed the <b>Step 8</b> in <b>Lab 0</b> to [ `Verify AKS cluster is provisioned` correctly!](/codelabs/azure-grail-lab0/index.html?index=..%2F..azure#5){target="_blank"}

!!! tip
    In <b>Step 6</b> of <b>Lab 0</b>, you should have also save off [two values ](/codelabs/azure-grail-lab0/index.html?index=..%2F..azure#5){target="_blank"} in a notepad session.  You will now need to input those values in this step.
...
1. Open up the Azure Portal and search for the AKS Cluster from the top search bar and select it once it displays under resources
    ![lab2 aks search](img/azure-grail-lab2/lab2-aks-search.png)
...
1. Click on `Install an extension`.
    ![lab2 aks dt extension install1](img/azure-grail-lab2/lab2-aks-dt-extension-install1.png)

1. Search Dynatrace in search bar.  Click on Dynatrace Operator tile once its displayed.  
    ![lab2 aks dt extension install2](img/azure-grail-lab2/lab2-aks-dt-extension-install2.png)
...
1. On the `Basics` tab, the subscription and resource group shold already be pre-selected. Just select the AKS Cluster from the drop down.
    ![lab2 aks dt extension install3](img/azure-grail-lab2/lab2-aks-dt-extension-install3.png)
...
1. On the `Dynatrace Operator Configuration` here are the values to fill in
    ...
    ![lab2 aks dt extension install4](img/azure-grail-lab2/lab2-aks-dt-extension-install4.png)
...
1. After the deployment is complete, go into Dynatrace -> From the left menu select `Apps` and bring up `Kubernetes Classic` app.
    ...
    ![lab2 aks dt extension install5](img/azure-grail-lab2/lab2-aks-dt-extension-install5.png)
...
!!! tip
    Before starting this step, please ensure you completed the `Step 6` in `Lab 0` to [ `Verify AKS cluster is provisioned` correctly!](/codelabs/azure-lab0/index.html?index=..%2F..azure#5){target="_blank"}
...
- Install Dynatrace Operator
    ...
    ![lab4 operator menu](img/azure-grail-lab2/lab4-operator-menu.png)
    ...
    1.6 Select the `Skip SSL Certificate Check` to be ON
        ...
        ![lab4 operator mod may2022](img/azure-grail-lab2/lab4-operator-mod-may2022.jpg)

        !!! tip
    📓 The `dynakube.yaml` file was automatically generated in Azure cloudshell during the provision script you ran in Lab0. Therefore we'll use a modified version of the commands        
...
!!! tip
    ...
    **📓`Shell Script to deploy sample app to Kubernetes`**
    ...
    By now you understand the various automation files, lets go ahead and open up the [ `start-k8.sh` ](https://github.com/dt-alliances-workshops/azure-modernization-dt-orders-setup/blob/master/app-scripts/start-k8.sh){target="_blank"} to review what the script did for you:
    ...
    - You can read more details on the Kubernetes installation in the [ Dynatrace Documentation ](https://dynatrace.com/support/help/platform-modules/infrastructure-monitoring/container-platform-monitoring/kubernetes-monitoring/leverage-tags-defined-in-kubernetes-deployments){target="_blank"}
...
!!! tip
    ...
    **📓 `Sample App YAML file for deploymenent`**
    <br>To review what is configured for the sample application, go ahead and click on the link for YAML file: [frontend.yml](https://github.com/dt-alliances-workshops/azure-modernization-dt-orders-setup/tree/master/app-scripts/manifests/frontend.yml){target="_blank"} 
...
![lab2 k8s namespaces](img/azure-grail-lab2/lab2-k8s-namespaces.png)
...
Kubernetes runs your workload by placing containers into Pods to run on [Nodes][ Nodes ](https://kubernetes.io/docs/concepts/architecture/nodes/){target="_blank"}.
...
1. Validate Sample App is accessible via browser
    ...
    3. From the output, copy the entire value from EXTERNAL-IP and 
        ![lab4 app ip](img/azure-grail-lab2/lab4-app-ip.png) 
...
1. In the Dynatrace UI, Go to Apps list from menu on the left and open up `Kubernetes Classic" 
    ![lab2 k8classic app](img/azure-grail-lab2/lab2-k8classic-app.png)
1. Open up the AKS Cluster
    ![lab2 k8classic cluster view](img/azure-grail-lab2/lab2-k8classic-cluster-view.png)
1. Once you're in Overview of Kubernetes cluster, scroll down to bottom to Events and Logs section
    ![lab2 k8classic cluster view eventslogs](img/azure-grail-lab2/lab2-k8classic-cluster-view-eventslogs.png)
1. Click on the button first to `Enable Kuberenetes Events`.  On the next screen click on Monitor events and click Save bottom.
    ![lab2 k8classic events config](img/azure-grail-lab2/lab2-k8classic-events-config.png)
1. To navigate back to your cluster overview screen, click on cluster name at the breadcumb menu at the top
    ![lab2 k8classic events config breadcrumb](img/azure-grail-lab2/lab2-k8classic-events-config-breadcrumb.png)   
...
1. Scroll to the top of the log ingest screen and enable `Ingest all detected log sources`.  
    ![lab2 k8classic logingest](img/azure-grail-lab2/lab2-k8classic-logingest.png)
...
2. Review Kubernetes Dashboards are accessible from the left-side menu in Dynatrace choose `Apps --> Kubernetes Classic` and navigate to the Kubernetes cluster page as shown below: <br>
    ...
    ![lab2 k8s layers upd](img/azure-grail-lab2/lab2-k8s-layers-upd.png)
    ...
    **2 - Nodes** - The resources for the Cluster are summarized for the one-to-many hosts or Cluster nodes in this view.
    ...
    ![lab2 aks nodeutiliz](img/azure-grail-lab2/lab2-aks-nodeutiliz.png)
    ...
    **3 - Namespaces** - Namespaces are ways to partition your cluster resources to run multiple workloads (for example `application A` and `application B` workloads) on same cluster
    1.	This `Namespace analysis` section shows workloads metrics over time
            ![lab2 aks namespace upd](img/azure-grail-lab2/lab2-aks-namespace-upd.png)
    2.	In the `Namespace Analysis` section, pick the view all `staging` namespace.
            
    - In the filter, pick namespace then staging
            ![lab2 aks staging filter](img/azure-grail-lab2/lab2-aks-staging-filter.png)

    **4 - Kubernetes workload**
    - Pick the frontend to drill into.
        ![la2 aks kubeworkload](img/azure-grail-lab2/la2-aks-kubeworkload.png)
    ...
    - Click on Kubernetes POD to look at POD utilization metrics.
        ![lab2 aks frontend workload](img/azure-grail-lab2/lab2-aks-frontend-workload.png)
    ...
    **5 - POD** - Review the POD overview page to look at various metrics related to the POD
    - Click on Container next to look at container metrics      
        ![lab2 aks pod](img/azure-grail-lab2/lab2-aks-pod.png)
     
    ...
    **6 - Containers** - Referring to the diagram above, expand the properties and tags section to view:
    ...
    ![lab2 aks container](img/azure-grail-lab2/lab2-aks-container.png)
    ...
    **7 - Service** - This view should now look familiar. In Lab 1, we looked at the service for the frontend and backend.  
            ![aks layer7 service upd](img/azure-grail-lab2/aks-layer7-service-upd.png)
...
1. Review Services View for `order` service
    * Left Menu -> Applications & Microservices -> Services <br>
        ![lab2 step8 services](img/azure-grail-lab2/lab2-step8-services.png)
    * Top Menu -> filter by ``` dt-orders-k8 management ``` zone.
        ![lab4 k8 mgmtzone filter](img/azure-grail-lab2/lab4-k8-mgmtzone-filter.png)
    * Pick the ``` order ``` service.
        ![lab4 k8 service filter](img/azure-grail-lab2/lab4-k8-service-filter.png)
    ...
    * Referring to the picture, within the services infographic, click on the "services" square to get a list of the services that the order service calls.
        ![lab2 k8 service view mod](img/azure-grail-lab2/lab2-k8-service-view-mod.png)
...
2. Review Backtrace View
    ...
    * This will get more interesting in the next lab, but for the monolith backend, we can see that the backtrace is as follows:
    ...
    1. My web application is the default application that Dynatrace creates
        ![lab4 k8 service backflow](img/azure-grail-lab2/lab4-k8-service-backflow.png)
...
1. Review Services View for `Frontend` service
    ...
    * Pick the ``` frontend ``` service.
        ![lab4 k8 frontendservice filter](img/azure-grail-lab2/lab4-k8-frontendservice-filter.png)
    * Just click on the `service flow` button to open this.
        ![lab2 serviceflow](img/azure-grail-lab2/lab2-serviceflow.png) 
...
2. Analyze Services view from Response time perspective
    ...
    * Order service calls order and customer service
    - Something you would never know from the application web UI!
        ![lab4 serviceflow responsetime](img/azure-grail-lab2/lab4-serviceflow-responsetime.png)
...
3. Analyze Services view from Throughput perspective
        ![lab4 serviceflow thoroughput](img/azure-grail-lab2/lab4-serviceflow-thoroughput.png)   
